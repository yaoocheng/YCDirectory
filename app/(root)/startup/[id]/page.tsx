...
async function page({ params }: { params: { id: string } }) {
    ...
    const response = await fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/startup-query/${id}`, {
    ...
    const sameResponse = await fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/startup-same/${id}`, {
        cache: 'no-store' // 确保每次都获取最新数据
    });
